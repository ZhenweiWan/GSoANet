...
class NextVLAD(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(times):
            x_i = x[i]
            x_i = x_i.reshape(x_i.size(0), x_i.size(1), x_i.size(2) * x_i.size(3))
            ...
            if self.normalize_input:
                x_i = F.normalize(x_i, p=2, dim=2)  # across descriptor dim
            x_i = self.fc_inp(x_i)
            alpha_g = torch.sigmoid(self.fc_alpha_g(x_i))
            alpha_gk = self.softmax(self.fc_alpha_gk(x_i))
            alpha_gk = alpha_gk.view(bs, M, self.groups, self.num_clusters)
            reshaped_x = x_i.view(bs, M, self.groups, self.expansion * N // self.groups)
            ...
            vlad_all.append(vlad)
        vlad_all = torch.mean((torch.stack(vlad_all, dim=-1)), dim=-1)
