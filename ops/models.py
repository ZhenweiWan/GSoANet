...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 temporal_pool=False, non_local=False, redu_dim=512):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'resnet' in base_model:
            ...
            from ops.nextvlad_so.mynextvlad_so import NextVLAD
            self.base_model.stages[3].add_module('vlad',
                                                 NextVLAD(num_clusters=48, dim=768, groups=8, expansion=2,
                                                          fusion_mode='sum'))
            self.base_model.norm = nn.LayerNorm(18528)
            # self.base_model.norm = nn.Identity()
            # self.base_model.downsample_layers[2][1].stride = 1
            self.base_model.downsample_layers[3][1].stride = 1
    ...
    def get_optim_policies(self):
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d):
                ...
                ps = list(m.parameters())
                conv3d_weight.append(ps[0])
                if len(ps) == 2:
                    conv3d_bias.append(ps[1])

            elif isinstance(m, torch.nn.Linear):
                ps = list(m.parameters())
                fc_cnt += 1
                if fc_cnt == 37 or fc_cnt == 38 or fc_cnt == 39 or fc_cnt == 40 \
                        or fc_cnt == 41:
                    new_fc_weight.append(ps[0])
                    if len(ps) == 2:
                        new_fc_bias.append(ps[1])
                else:
                    if self.fc_lr5:
            ...
            elif isinstance(m, torch.nn.BatchNorm2d) or isinstance(m, models.convnext.LayerNorm) or \
                    isinstance(m, torch.nn.modules.normalization.LayerNorm):
                bn_cnt += 1
                if bn_cnt == 23:
                    new_ln.extend(list(m.parameters()))
                # later BN's are frozen
                elif not self._enable_pbn or bn_cnt == 1:
            ...
            # elif isinstance(m,ops.temporal.vlad.SeqVLADModule):
            #     vlad.extend(list(m.parameters()))
            elif len(m._modules) == 0:
                if len(list(m.parameters())) > 0:
                    raise ValueError("New atomic module type: {}. Need to give it a learning policy".format(type(m)))

        return [
            {'params': first_conv_weight, 'lr_mult': 1 if self.modality == 'Flow' else 1, 'decay_mult': 1,
             'name': "first_conv_weight"},
            {'params': first_conv_bias, 'lr_mult': 2 if self.modality == 'Flow' else 2, 'decay_mult': 0,
             'name': "first_conv_bias"},

            {'params': conv_weight, 'lr_mult': 10, 'decay_mult': 1,
             'name': "conv_weight"},
            {'params': conv_bias, 'lr_mult': 20, 'decay_mult': 1,
             'name': "conv_bias"},
            ...
            {'params': custom_ops, 'lr_mult': 1, 'decay_mult': 1,
             'name': "custom_ops"},
            # for temporal
            {'params': conv3d_weight, 'lr_mult': 10, 'decay_mult': 1,
             'name': "conv3d_weight"},
            {'params': conv3d_bias, 'lr_mult': 20, 'decay_mult': 0,
             'name': "conv3d_bias"},
            # for fc
            ...
            {'params': lr10_bias, 'lr_mult': 2, 'decay_mult': 0,
             'name': "lr10_bias"},
            {'params': new_fc_weight, 'lr_mult': 10, 'decay_mult': 1,
             'name': "new_fc_weight"},
            {'params': new_fc_bias, 'lr_mult': 20, 'decay_mult': 0,
             'name': "new_fc_bias"},
            {'params': new_ln, 'lr_mult': 10, 'decay_mult': 0,
             'name': "new_ln"},
        ]
